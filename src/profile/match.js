...
export default function Match({matchId, api_key, profileId}){
    ...
    if(match != undefined){
        ...
        const gameOutcome = (match.info.teams[0].teamId == playerTeam && match.info.teams[0].win == true) || (match.info.teams[1].teamId == playerTeam && match.info.teams[1].win == true) ? "Win" : "Loss" //playerTeam == 100 ? "Win" : "Loss"
        const gameStartDate = new Date(match.info.gameCreation).toISOString().slice(0,10)
        const gameStartTime = new Date(match.info.gameCreation).toISOString().slice(11,16)
        ...
        return(
            <div className={style.matchContainer}>
                <h2>{gameMode}</h2>
                <h1>{gameOutcome}</h1>
                <h3>{gameStartDate} {gameStartTime}</h3>
                <h3>{gameDuration}</h3>
            </div>
        )
    }
